...
export const assetFromJSON = (asset: any): OpenSeaAsset => {
  ...
  return groupAndAggregateOrders(fromJSON, asset) as OpenSeaAsset
}
...
export const assetBundleFromJSON = (asset_bundle: any): OpenSeaAssetBundle => {
  const fromJSON: OpenSeaAssetBundle = {
    ...
    sellOrders: null,
    buyOrders: null,
    orders: asset_bundle.orders ? asset_bundle.orders.map(orderFromJSON) : null,
  }

  return groupAndAggregateOrders(fromJSON, asset_bundle) as OpenSeaAssetBundle
}

export const groupAndAggregateOrders = (
  assetOrBundleFromJson: OpenSeaAssetBundle | OpenSeaAsset,
  asset_or_bundle: any
): OpenSeaAsset | OpenSeaAssetBundle => {
  assetOrBundleFromJson.sellOrders = asset_or_bundle.sell_orders
    ? asset_or_bundle.sell_orders.map(orderFromJSON)
    : null
  assetOrBundleFromJson.buyOrders = asset_or_bundle.buy_orders
    ? asset_or_bundle.buy_orders.map(orderFromJSON)
    : null

  // If orders were included, put them in sell/buy order groups
  if (assetOrBundleFromJson.orders && !assetOrBundleFromJson.sellOrders) {
    assetOrBundleFromJson.sellOrders = assetOrBundleFromJson.orders.filter(
      (o: Order) => o.side == OrderSide.Sell
    )
  }
  if (assetOrBundleFromJson.orders && !assetOrBundleFromJson.buyOrders) {
    assetOrBundleFromJson.buyOrders = assetOrBundleFromJson.orders.filter(
      (o: Order) => o.side == OrderSide.Buy
    )
  }
  return assetOrBundleFromJson
}
