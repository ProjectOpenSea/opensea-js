...
import { confirmTransaction, encodeCall,
  feeMethod, feeRecipient, findAsset, howToCall,
  makeBigNumber, orderSide, orderToJSON,
  personalSignAsync, promisify, saleKind,
  sendRawTransaction } from './wyvern';

export default class OpenSea {

  constructor(provider: Web3Provider, { networkName = 'main', gasPrice = 100000 }) {
    // Web3 Config
  ...
  // Throws
  public async _validateSellOrderParameters({ order, accountAddress }) {
    const schema = this._getSchema();
  ...
  // Throws
  public async _validateBuyOrderParameters({order, accountAddress}) {
