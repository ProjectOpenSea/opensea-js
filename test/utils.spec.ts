import { CROSS_CHAIN_SEAPORT_V1_5_ADDRESS, CROSS_CHAIN_SEAPORT_V1_6_ADDRESS } from "@opensea/seaport-js/lib/constants";
...
suite("Utils: utils", () => {
  test("isValidProtocol works with all forms of address", async () => {
    ...
    const formatsToCheck = (address: string) => [address, address.toLowerCase(), ethers.getAddress(address)]

    for (const [address, isValid] of addressesToCheck) {
      for (const formattedAddress of formatsToCheck(address)) {
        assert(isValidProtocol(formattedAddress), isValid);
      }
    }
  });
});
