...
exports.getMethod = function (abi, name) {
    var methodAbi = abi.find(function (x) { return x.type == web3_1.AbiType.Function && x.name == name; });
    if (!methodAbi) {
...
exports.event = function (abi, name) {
    var eventAbi = abi.find(function (x) { return x.type == web3_1.AbiType.Event && x.name == name; });
    if (!eventAbi) {
        throw new Error("ABI " + name + " not found");
    }
    // Have to cast since there's a bug in
