 ...
 */
function getWyvernBundle(schema, assets, quantities) {
    if (assets.length != quantities.length) {
        throw new Error("Bundle must have a quantity for every asset");
    }
    var wyAssets = assets.map(function (asset, i) { return getWyvernAsset(schema, asset, quantities[i]); });
    var sorters = [function (a) { return a.address; }, function (a) { return a.id || 0; }];
