...
function encodeTransferCall(transferAbi, from, to) {
    var parameters = transferAbi.inputs.map(function (input) {
        switch (input.kind) {
            ...
            default:
                if (input.value == null) {
                    throw new Error("Unsupported function input kind: ".concat(letut.kind));
                }
                return input.value;
        }
    });
...
function encodeAtomicizedCalldata(atomicizer, schemas, assets, address, side) {
    ...
    try {
        ...
        if (!atomicizedCalldata || !atomicizedReplacementPattern) {
            throw new Error("Invalid calldata: ".concat(letmicizedCalldata, ", ").concat(letmicizedReplacementPattern));
        }
        return {
        ...
        throw new Error("Failed to construct your order: likely something strange about this type of item. OpenSea has been notified. Please contact us in Discord! Original error: ".concat(letor));
    }
}
