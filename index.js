...
import { feeRecipient, findAsset,
  orderToJSON, encodeCall, promisify, orderSide,
  saleKind, howToCall, feeMethod,
  makeBigNumber, sendRawTransaction, confirmTransaction,
  personalSignAsync } from './wyvern'
import { WyvernProtocol } from 'wyvern-js/lib'
import * as WyvernSchemas from 'wyvern-schemas'
import { ERC721, getMethod, ERC20, CanonicalWETH, DECENTRALAND_AUCTION_CONFIG } from './contracts';
import OpenSeaAPI from './api';

export default class OpenSea {

  constructor(web3Provider, { networkName = "main", gasPrice = 100000 }) {
    // Web3 Config
    ...
    // WyvernJS config
    this.wyvernProtocol = new WyvernProtocol(this.web3Provider, {
  ...
  async wrapEth({amountInEth, accountAddress, awaitConfirmation = true}) {
    ...
    return await sendRawTransaction(this.web3, {
  ...
  async unwrapWeth({amountInEth, accountAddress, awaitConfirmation = true}) {
  ...
  async createBuyOrder({ tokenId, tokenAddress, accountAddress, amountInEth, expirationTime = 0 }) {
    const token = WyvernSchemas.tokens[this.networkName].canonicalWrappedEther
    ...
    const order = {
      ...
      feeMethod: feeMethod.PROTOCOL_FEE,
    ...
    return await this._validateAndPostOrder(orderJSON)
  }

  async createSellOrder({ tokenId, tokenAddress, accountAddress, startAmountInEth, endAmountInEth, expirationTime = 0 }) {
    const schema = this._getSchema()
    const wyAsset = this._getWyvernAsset(schema, {tokenId, tokenAddress})
    const metadata = {
      asset: wyAsset,
  ...
  async getApprovedTokenCount({accountAddress, tokenAddress}) {
  ...
  async approveNonFungibleToken({tokenId, tokenAddress, accountAddress, proxyAddress, tokenAbi = ERC721}) {
    ...
    var approvedAddr = await promisify(c => erc721.getApproved.call(tokenId, c))
  ...
  async approveFungibleToken({accountAddress, tokenAddress}) {
    const contractAddress = WyvernProtocol.getTokenTransferProxyAddress(this.networkName)
  ...
  async _atomicMatch({ buy, sell, accountAddress }) {
    const protocolInstance = this.wyvernProtocol
  ...
  async _makeMatchingOrder({ order, accountAddress }) {
    const schema = this._getSchema()
  ...
  async _getProxy(accountAddress) {
    const protocolInstance = this.wyvernProtocol
  ...
  // Throws
  async _validateSellOrderParameters({ order, accountAddress }) {
    const schema = this._getSchema()
    const wyAsset = order.metadata.asset
    let proxyAddress = await this._getProxy(accountAddress)
    const where = await findAsset(accountAddress, proxyAddress, wyAsset, schema)
    if (where === 'other') {
  ...
  // Throws
  async _validateBuyOrderParameters({order, accountAddress}) {
  ...
  async _getTokenBalance({accountAddress, tokenAddress, tokenAbi = ERC20}) {
    if (!tokenAddress) {
  ...
  // Throws
  async _validateAndPostOrder(order) {
    const protocolInstance = this.wyvernProtocol
  ...
  _getWyvernAsset(schema, {tokenId, tokenAddress}) {
